...
`;
		...
		/**
		 * Warn when the host half is not wired to the continuation seam. The usual
		 * cause is a profile that never added the official Agent Teams layer, so
		 * that case names the missing package instead of asking for a restart that
		 * could not help.
		 */
		function hookWarning(view) {
			if (!view || !view.hook || view.hook.installed === true) return null;
			if (view.hook.agentTeams === false) {
				return "未检测到官方 Agent Teams 上层插件：请先运行 dsh plugin --profile web add "
					+ "@deepseek-ai/dsh-experimental-agent-team-profile（桌面端换成自己的 profile 名），再重启 DSH。";
			}
		...
		function completedCountLabel(view) {
			const teammates = (view?.members ?? []).filter((member) => member?.role === "teammate");
			return `${teammates.filter((member) => memberTaskState(member) === "completed").length}/${teammates.length} 个队员已完成`;
		}
		...
		/** Project residency plus durable turn history into one teammate task state. */
		function memberTaskState(member) {
			if (member?.status === "failed") return "failed";
			if (member?.status === "running" || member?.status === "provisioning") return "executing";
			if (member?.workState === "completed") return "completed";
			return "standby";
		}

		function memberWorkLabel(member) {
			const state = memberTaskState(member);
			if (state === "executing") return "正在执行任务";
		...
		function memberWorkDot(member) {
			const state = memberTaskState(member);
			if (state === "executing") return "ongoing";
		...
		function memberRow(key, member, nameNode, note, lead) {
			return h("div", { key, className: lead === true ? "atm_row atm_leadRow" : "atm_row" }, [
				h(StateDot, { key: "dot", state: lead === true ? memberDotState(member.status) : memberWorkDot(member), size: 8 }),
		...
		function SessionModelConfig(props) {
			...
			if (rosterError !== null) {
				...
				teammates.forEach((member, index) => {
					memberRows.push(memberRow(member.id || index, member, member.name || member.id || "—", `队员 · ${memberWorkLabel(member)}`, false));
				});
			}
		...
		function SessionComposerEntry(props) {
			...
			// A failed roster read is an error state of this plugin, not an empty team:
			// the usual cause is a profile without the official Agent Teams layer, so
			// the reason (which names the package) belongs on the chip, not only
			// inside the dialog.
			const rosterError = view !== null && typeof view.membersError === "string" && view.membersError !== "" ? view.membersError : null;
			const emptyTeam = view !== null && !teammate && !error && rosterError === null
			...
			const label = view === null ? (error ? "队员 · 读取失败" : "队员 · 加载中…")
				...
				: rosterError !== null ? "队员 · 读取失败" : emptyTeam ? "" : completedCountLabel(view);
			const teammateStates = (view?.members ?? []).filter((row) => row?.role === "teammate").map(memberTaskState);
			const state = teammate ? memberWorkDot(member)
				: rosterError !== null ? "error"
				: teammateStates.some((value) => value === "executing") ? "ongoing"
					: teammateStates.length > 0 && teammateStates.every((value) => value === "completed") ? "done" : "idle";
			return h("span", { className: "atm_entry" }, [
				h(Button, {
					...
					title: error ? `读取失败：${error}` : view === null ? "正在读取当前对话的队员信息"
						: teammate ? "查看当前队员自己的模型与推理强度（只读）"
						: rosterError !== null ? `队员 · 读取失败：${rosterError}`
						: emptyTeam ? "队员配置" : `${label}；查看团队与新建队员路由配置`,
